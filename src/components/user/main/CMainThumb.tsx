import { Box, IconButton, ListItem, ListItemButton, TextareaAutosize, styled } from "@mui/material"
import CButton from "@/components/CButton"
import { MoreHoriz, FavoriteBorder, TurnedInNot, ChatBubbleOutline, Favorite } from '@mui/icons-material';
...
const CMainThumb = () => { 
  ...
  const getListData = useCallback(async() => { 
    ...
    if(res.data.code === 200) { 
      ...
      // 게시물 랜더링
      res.data.result.filter((item: {account_name:string, list_no: number, list_content: string}) => {
        if(item.account_name === path.split('/')[1] && item.list_no === Number(path.split('/')[2])){ 
  ...
  // 게시글 수정하기 
 const handleListUpdate = async(account_name: string, list_no: number, content: string) => {
  if(content === "") { 
  ...
  const handleLikeRemove = async(account_name: string, list_no: number) => {   
    await axios({
      method: 'delete', 
      url: `${import.meta.env.VITE_BACK_URL}/list/likes/${account_name}/${list_no}`,  
      withCredentials: true
    }) 
    .then(() => {
      setLike(false)
           ...
           <MainBox key={item.list_no}> 
              ...
              <ContentBox>
                <ThumbBox>
                  ...
                  <Box>
                    {
                      user.isAuth && user.account_name !== path.split('/')[1] ? <CButton
                      style={FollowButton} 
                      type="lightgray" 
                    ...
                    {  
                      ...
                      <ListMenu>
                        <ListItem>
                          <ListItemButton
                            disableRipple  
                            onClick={() => handleListDelete(item.account_name, item.list_no)}
                          >  
                            <p>삭제하기</p>
                          </ListItemButton> 
                        </ListItem>
