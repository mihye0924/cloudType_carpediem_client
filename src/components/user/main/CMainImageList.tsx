...
const CMainImageList = () => {   
  ...
  const handleImageList = async(item: listType) => {
    ...
    !user.isAuth ? setAlert("로그인이 필요합니다") : navigate(`/${item.account_name}/${item.list_no}`) 
    // listGoodSave(item)
  }
 
  return (
    <Section className={`${ !user.isAuth ? 'not_login' : ''}${ user.isAuth && profile.account_link ? 'height30':''}${ user.isAuth && profile.account_info ? 'height30':''}${ user.isAuth && profile.account_link && profile.account_info ? 'height40': ''}`}>
      {    
        list.length > 0 ? 
        <>
          <ListImage>
