...
const JoinPage = () => { 
  ...
  const pwValueCheck = (str: string) => {
    const pwRegex = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(str)
    setPwText(pwRegex) 
  } 
  ...
  const birthValueCheck = (str: string) => { 
    ...
    dateRegex.test(str) || dateRegex2.test(str) ? setBirthText(true) : setBirthText(false) 
  }
  ...
  const emailValueCheck = (str: string) => {
    const emailRegx = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*\.[a-zA-Z]{3}$/i.test(str)
    ...
    >
      <Box sx={Container}> 
        ...
        <Box sx={InputInner}>
          ...
          {
            (!idText && id.length > 0) &&
            <Box sx={ErrorText}>아이디는 영문, 숫자 4~10자리로 입력해야합니다!</Box>
          } 
        <Box>
          <TextField 
            sx={FormInputPw} 
        ...
        {
          (!pwText && pw.length > 0) &&
          <Box sx={ErrorText}>8글자 이상, 영문, 숫자, 특수문자로 입력해주세요.</Box>
        } 
        <Box>
          <TextField 
            sx={FormInputPwCheck} 
            ...
            value={pwCheck}
            ref={pwCheckRef} 
            label="재확인 비밀번호를 입력해주세요" variant="outlined" 
          />  
        </Box>
        {
        ...
        <Box>
          <TextField 
            ...
            value={birth}
            ref={birthRef}
            label="YYYY-MM-DD형식으로 입력해주세요" variant="outlined" 
          />    
          {
        ...
        <Box>
          <TextField 
            ...
            value={email}
            ref={emailRef}
            label="이메일을 입력해주세요" variant="outlined" 
          />    
        </Box>
        {
